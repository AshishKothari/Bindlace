...
import { measureIteration } from "./collectMetrics.mjs";
import { variants } from "./variants.mjs";
import { mean, percentile } from "./stats.mjs";

function parseArgs(argv) {
  const args = { variant: "", iterations: 10 };
...
async function main() {
  ...
  const run = make();
  const baseOut = args.outDir ? path.resolve(args.outDir) : path.resolve("bench", "out", isoStampForPath());
  ...
  for (let i = 1; i <= args.iterations; i++) {
    ...
    const proc = r.procSample;
    let externalPeakWsBytesApproxSum;
    let externalCpuDeltaSecondsSum;
    if (proc && proc.byName && typeof proc.byName === "object") {
      externalPeakWsBytesApproxSum = 0;
      externalCpuDeltaSecondsSum = 0;
      for (const v of Object.values(proc.byName)) {
  ...
  const extWs = rows.map((x) => x.externalPeakWsBytesApproxSum).filter((x) => typeof x === "number");
  const extCpu = rows.map((x) => x.externalCpuDeltaSecondsSum).filter((x) => typeof x === "number");
